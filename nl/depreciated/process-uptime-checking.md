---
title: Proces Uptime Controle (door BamBam)
description: This is a process uptime checking utility.
published: true
date: 2022-11-20T07:28:50.946Z
tags: 
editor: markdown
dateCreated: 2022-07-24T16:09:42.464Z
---

# Proces Uptime Controle [(door BamBam)](https://www.twitch.tv/bambam4275)

## Beschrijving

Dit is een proces uptime controle hulpmiddel.  Als u het configureert, dan zal het het proces uptime naar een bestand in dit formaat beschrijven `Uptime: 01.03:41:15` `days.hours:min:seconden` formaat.

## Installatie
Kopieer de import code van onderaan en plak deze in de `Import venster`:
```text

```
![process-uptime-checking-import.png](/depreciated/process-uptime-checking/images/process-uptime-checking-import.png)

## Configuratie
Wijzig de `Sub-actie` `Zet Argument` `processUptimeSavePath` om overeen te komen met uw bestandspad. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-file-path.png)

Wijzig de `Sub-actie` `Zet Argument` `processUptimeSavePath` om overeen te komen met uw bestandspad. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-file.png)

Wijzig de `Sub-actie` `Zet Argument` `processUptimeSavePath` om overeen te komen met uw bestandspad. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-process.png)

### References
In de `Execute C# Code` `Sub-actie` zorg je ervoor dat je de volgende referenties hebt: ![process-uptime-checking-references.png](/depreciated/process-uptime-checking/images/process-uptime-checking-references.png)
