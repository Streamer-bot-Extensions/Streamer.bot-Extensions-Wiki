---
title: Convertidor de valores en código hexadecimal y RGB (por ConfuzzedCat)
description:
published: true
date: 2022-07-24T06:09:29.698Z
tags: confuzzedcat, hex code, hex, rgb, rgb colors, convert, converter, hex to rgb, rgb to hex, hex2rgb, rgb2hex
editor: markdown
dateCreated: 2022-05-18T01:21:09.750Z
---

# Convertidor de valores en código hexadecimal y RGB [(por ConfuzzedCat)](https://www.twitch.tv/confuzzedcat)
## Descripción
Una acción simple para convertir valores RGB a códigos hexadecimales, y a la inversa.

## Código de importación
```

```
## Instalación
En Streamer.bot, en el panel de `Importación` de la parte superior izquierda, copia el `Código de Importación` y pégalo en la `Cadena de Importación`.

## Configuración
### Hexadecimal a RGB
Puedes convertir uno o dos colores. Si solo estás convirtiendo un color, el formato puede ser ya sea `!cconvert hexcode` o `!cconvert #hexcode`, por ejemplo: `!cconvert 12FF19` o `!cconvert #12FF19`. Si estás convirtiendo dos colores, el primer hexadecimal requiere tener un `#` en frente del código hexadecimal; el segundo código no lo requiere, pero funcionará con él, por ejemplo: `!cconvert #12FF19 12FF19` o `!cconvert #12FF19 #12FF19`.

### RGB a Hexadecimal
A diferencia del hexadecimal a RGB, solo puedes convertir un color a la vez. El formato es `!cconvert 100, 32, 200` o `!cconvert 100, 32, 200 true`, donde "true" dicta si el código hexadecimal resultado tendrá el símbolo gato `#`.

## Uso
Mientras el formato sea correcto, la acción detectará automáticamente si se trata de un valor hexadecimal o RGB y lo convertirá.
```
!cconvert 191919
Resulta en: 25, 25, 25
!cconvert #191919 FFFFFF
Resulta en: 25, 25, 25 | 255, 255, 255
!cconvert 255, 255, 255
Resulta en: FFFFFF
!cconvert 191, 66, 240 true
Resulta en: #BF42F0 
```

