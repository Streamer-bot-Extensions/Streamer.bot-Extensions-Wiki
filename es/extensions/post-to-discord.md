---
title: Publicar en Discord (por Geocym)
description: Permite a Streamer.bot publicar en Discord.
published: true
date: 2022-07-24T06:10:01.822Z
tags:
editor: markdown
dateCreated: 2022-04-19T02:26:59.599Z
---

# Publicar en Discord [(por Geocym)](https://www.twitch.tv/geocym)
## Descripción
Con esta extensipon, podrás publicar en Discord. Esto puede incluir cualquier cosa desde imágenes hasta clips. Incluso puede ser usado para publicar un mensaje de inicio de transmisión.

## Código de Importación
```text

```

## Instalación
En Streamer.bot, en el panel de `Importación` de la parte superior izquierda, copia el `Código de Importación` y pégalo en la `Cadena de Importación`.

## Configuración

### Discord
En tu servidor de Discord, ve a `Server Settings> Integrations` y haz click en `New Web Hook`. ![post-to-discord-1.png](/extensions/post-to-discord/images/post-to-discord-1.png)

Elige un canal en el cual publicar y darle un nombre, cualquier cosa funcionará. ![post-to-discord-1.png](/extensions/post-to-discord/images/post-to-discord-2.png)

Copia la URL del Webhook, necesitarás esto para el código de C# más adelante, así que guárdalo en algún lugar por ahora.

### Streamer.bot
Cambia la `Sub-acción` `OBS Raw (Take Screenshot)` para que coincida con tu configuración. ![post-to-discord-obs-raw.png](/extensions/post-to-discord/images/post-to-discord-obs-raw.png)

En la sub-acción `C# Execute Code` inserta el Webhook de Discord y cambia la ruta del archivo en donde lo indica.
```cs
string Webhook_link = "PEGA LA URL DE TU WEBHOOK AQUÍ";
              // Enter the path to match your screenshot
    string FilePath = @"D:\OBS\Discord\screenshot.png";
 ```

 #### References Asegúrate de que tengas las siguientes referencias agregadas a la sub-acción `Execute C# Code` y luego presiona `Save and Compile`. ![post-to-discord-obs-references.png](/extensions/post-to-discord/images/post-to-discord-references.png)