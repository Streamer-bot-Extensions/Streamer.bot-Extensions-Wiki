...
# Juego de Adivinizanza de Emotes [(por @Kleinhalbi)](https://www.twitch.tv/Kleinhalbi)

## Descripción
Por solicitud de @Dewaldsch creé un Juego de Adivinanza de Emotes. Funciona de la siguiente manera:
* Tienes un archivo cont odos los emotes globales de Twitch
* El bot elige uno alazar que deberá ser adivinado por el resto de los usuarios (el mismo emote no será elegido dos veces por el bot, a menos que haya pasado por toda la lista)
* El chat podrá será puesto en modo de Solo Emotes y modo lento (por 5 segundos)
* El juego terminará cuando un usuario escriba el emote correcto, o después de que hayan pasado 10 minutos; lo que ocurra primero
...
Asegúrate de que la `Acción` llamada `"03 Check chat messages"` NO esté habilitada. Crea un nuevo archivo de texto con todos los emotes que desees utilizar. Aquí hay una lista de todos los emotes globales de Twitch:
```text
4Head
:(
:)
