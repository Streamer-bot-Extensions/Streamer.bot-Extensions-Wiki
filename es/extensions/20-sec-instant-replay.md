---
title: Repetición Instantánea de 20 Segundos (por MarkusoOoO)
description: Crear una repetición instantánea de 20 segundos con Streamer.bot.
published: true
date: 2022-11-20T07:25:36.513Z
tags: repetición, rebobinado, efecto
editor: markdown
dateCreated: 2022-07-30T04:40:32.316Z
---

# Código de Importación

```text

```
# Instalación
En Streamer.bot, en el panel de `Importación` de la parte superior izquierda, copia el `Código de Importación` y pégalo en la `Cadena de Importación`. ![20-sec-instant-replay.png](/assets/20-sec-instant-replay/images/20-sec-instant-replay.png)

Puedes seguir el [video](https://youtu.be/NuWKrpNCE1k) de Nutty para configurar OBS con al menos `escenas anidadas` & `filtro de retraso dinámico`. Todo el efecto consume aproximadamente ` 8 GB de tu memoria RAM`, así que tenlo en cuenta.

Además, asegúrate que todas las `Sub-Acciones` apuntan hacia tus escenas y filtros de OBS, así como tus archivos de sonidos.
> Planeo hacer un video mostrando como configurar todo esto de inicio a fin.{.is-info}


# Recursos
¡Todos los recursos pueden ser encontrados [aquí](/assets/20-sec-instant-replay/files/Rewind-Effect.zip)!


# Configuración
Para obtener los tiempos correctamente con mi acción de ejemplo, usa la siguiente configuración para el plugin `Retraso Dinámico`:

|                            Configuración |         Datos          |
| ----------------------------------------:|:----------------------:|
|                                Duration: |    `20s`{.datatype}    |
|                            Fast Forward: |   `1000%`{.datatype}   |
|                           Fast Backward: |  `-1000%`{.datatype}   |
|   Tecla Rápida en OBS para Fast Forward: | `CTRL+NUM3`{.datatype} |
| Tecla Rápida en OBS para Fast Backwards: | `CTRL+NUM1`{.datatype} |

# Colaboradores

 - [MarkusoOoO](https://www.twitch.tv/markusoooo){.twitch}
 {.contributors}

