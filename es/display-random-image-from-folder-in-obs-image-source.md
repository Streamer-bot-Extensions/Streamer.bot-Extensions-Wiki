---
title: Display Random Image from Folder in OBS Image Source (by iAmLarcyn)
description: Display a Random Image from Folder in OBS Image Source with Streamer.bot.
published: true
date: 2022-07-24T04:58:39.930Z
tags: 
editor: markdown
dateCreated: 2022-07-24T00:41:38.488Z
---

# Display Random Image from Folder in OBS Image Source (by iAmLarcyn)

## Import Code
```

```

## Installation
1) in OBS, create an image source and name it. 2) In Streamerbot, import the code. 3) Change the arguments to the folder path containing images (only images), and the name of the obs source. From there, you can adjust how you show/hide the source however you would like.
> If it doesn't work, you may have to manually bring in the System.IO and System.Drawing references in the Execute code step {.is-warning}