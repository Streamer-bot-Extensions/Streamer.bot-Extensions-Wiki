---
title: Überprüfung der Prozesslaufzeit (von BamBam)
description: Dies ist ein Dienstprogramm zur Überprüfung der Betriebszeit von Prozessen.
published: true
date: 2022-11-20T07:16:37.947Z
tags:
editor: markdown
dateCreated: 2021-06-17T00:29:22.011Z
---

# Überprüfung der Betriebszeit von Prozessen [(von BamBam)](https://www.twitch.tv/bambam4275)

## Beschreibung

Dies ist ein Dienstprogramm zur Überprüfung der Betriebszeit von Prozessen.  Wenn Sie es konfigurieren, wird die Betriebszeit Ihres Prozesses in eine Datei in folgendem Format geschrieben `Uptime: 01.03:41:15` `days.hours:min:seconds` Format.

## Installation
Kopieren Sie den Importcode von unten und fügen Sie ihn in das `Importfenster` ein:
```text
...
```
![process-uptime-checking-import.png](/depreciated/process-uptime-checking/images/process-uptime-checking-import.png)

## Konfiguration
Ändern Sie den `Sub-action` `Set Argument` `processUptimeSavePath` entsprechend Ihrem Dateipfad. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-file-path.png)

Ändern Sie die `Sub-action` `Set Argument` `processUptimeSaveFile` zu der Textdatei, in der Sie speichern möchten. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-file.png)

Ändern Sie `Sub-action` `Set Argument` `processUptimeName` zu den Namen des Prozesses, den Sie verfolgen möchten. ![process-uptime-checking-file-path.png](/depreciated/process-uptime-checking/images/process-uptime-checking-process.png)

### Referenzen
Vergewissern Sie sich in `Execute C# Code` `Sub-action` dass Sie die folgenden Referenzen haben: ![process-uptime-checking-references.png](/depreciated/process-uptime-checking/images/process-uptime-checking-references.png)
